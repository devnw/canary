---
description: Create a new BUG CANARY token and actionable bug report from natural-language input; enforce best-practice bug reporting; no-mock/no-simulate
---
...
Read `.canary/project.yaml` to determine the project key. Use the project key for all requirement ID prefixes (e.g., `BUG-<PROJECT_KEY>-NNN`).
...
* **User Arguments (raw):** `$ARGUMENTS` -- natural-language defect report from user/agent.
  ...
  **MANDATORY:** If `$ARGUMENTS` is empty, return `ERROR_DESCRIPTION_REQUIRED()`.
...
4. **Duplicate check (must run):**
   ...
   Identify candidate duplicates (same summary keywords; same files/stack; same component). If a high-similarity existing BUG is found, return as `DUPLICATE_CANDIDATE` with links; **do not create a new ID** unless user insists.
...
* **CG-1 Parse:** title, 5W's (who/what/when/where/why), severity/priority hints, environment, logs.
* **CG-2 Deduplicate:** run `canary grep` and compute similarity.
* **CG-3 Create:** run script to scaffold folder + `report.md`.
* **CG-4 Author:** fill **report.md** sections (see section 4) strictly from facts; **no speculation**; separate *Expected*/*Actual*; provide minimal reproducible steps and environment.
...
* **One issue per bug**; split unrelated issues into separate BUG IDs.
* **Required sections in `report.md` (exact headings):**

  1. **Bug Summary** (10 words or fewer, problem-focused; not a solution).
  ...
  4. **Steps to Reproduce** (numbered, minimal but complete; include key interactions).
  5. **Expected Result** / **Actual Result** (exact message, stack, screenshots/logs refs; facts, not speculation).
  6. **Reproducibility** (e.g., `3/5` attempts; note "intermittent" & conditions).
  7. **Scope & Impact** (user/business impact; data loss, security, perf).
  8. **Regression/First-Seen** (version/build range; suspected change / pushlog if known).
  9. **Workarounds** (if any).
  10. **Attachments** (sanitized logs, screenshots, reduced testcase URL/path).
  11. **Related/Dependencies** (BUG/REQ links).
  12. **Next Actions (Triage)** (owner/team, first fix ideas, test to add).
* **Tone & brevity:** Be factual, avoid blame; right-sized detail for dev audience; include visuals where useful.
...
* **Steps Gate:** numbered, precise, minimal repro steps.
* **Expected/Actual Gate:** both sections present; facts separated from speculation.
* **Env Gate:** OS/browser/app version captured.
* **Repro Gate:** `k/n` provided; note intermittent if needed.
* **Attachments Gate:** logs/screenshots or reduced testcase linked when useful.
