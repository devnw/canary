...
1. **Read project key**:
   - Read `.canary/project.yaml` to determine the project key (e.g., `<PROJECT_KEY>`)

2. **Parse search pattern**:
...
**Find by Requirement:**
```bash
canary grep <PROJECT_KEY>-API-120    # All tokens for a specific requirement
```
