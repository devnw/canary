...
// CANARY: REQ=CP-281; FEATURE="MCPServerCommand"; ASPECT=Wire; STATUS=TESTED; TEST=TestMCPCommandCreation; UPDATED=2026-08-29
...
The server runs as an HTTP endpoint that AI assistants can interact with.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			mcp.AddTool(server, &mcp.Tool{
				Name:        "list",
				Description: "List CANARY tokens with optional filtering (default limit 20, max 100 to reduce context; Total reports the true match count)",
			}, handleList)
			...
			mcp.AddTool(server, &mcp.Tool{
				Name:        "next",
				Description: "Identify next highest priority unimplemented requirement. External (ticket-source) dependencies with cached status resolve as satisfied/unsatisfied; with no cached status they are treated as non-blocking here (run 'canary ticket status --refresh' to populate the cache) -- unlike the CLI's `canary next`, which blocks on an unresolvable dependency unless --allow-unknown-external is passed.",
			}, handleNext)
...
func printServerInfo(addr string) {
	...
	fmt.Println()
	fmt.Println("Available MCP Tools (19 total):")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("One-Call Hierarchical Context:")
	fmt.Println("  - view               - Full picture of a requirement in one call")
	fmt.Println("  - deps               - Dependency IDs, forward or reverse")
	fmt.Println()
	fmt.Println("Workflow & Development:")
	fmt.Println("  - scan               - Scan codebase for CANARY tokens")
	fmt.Println("  - specify            - Create requirement specification (stub — not yet implemented)")
	fmt.Println("  - plan               - Generate implementation plan (stub — not yet implemented)")
	fmt.Println("  - implement          - Get implementation guidance")
	fmt.Println("  - index              - Index codebase tokens into database (stub — not yet implemented)")
	fmt.Println()
	...
	fmt.Println("  - bug-create         - Create new bug token (stub — not yet implemented)")
	fmt.Println()
	fmt.Println("Gap Analysis:")
	fmt.Println("  - gap-mark           - Mark gap claims as helpful/unhelpful (stub — not yet implemented)")
	fmt.Println()
