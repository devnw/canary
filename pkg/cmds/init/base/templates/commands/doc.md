...
```
/canary.doc create <PROJECT_KEY>-<ASPECT>-XXX --type user
```
...
/canary.doc update <PROJECT_KEY>-<ASPECT>-XXX
...
```
/canary.doc status <PROJECT_KEY>-<ASPECT>-XXX
...
```go
// CANARY: REQ=<PROJECT_KEY>-<ASPECT>-105; FEATURE="UserAuth"; ASPECT=API; STATUS=IMPL;
...
```
/canary.doc create <PROJECT_KEY>-<ASPECT>-105 --type user
```

**Agent should:**
1. Run `canary doc create <PROJECT_KEY>-<ASPECT>-105 --type user --output docs/user/authentication.md`
2. Edit the generated template with actual content
3. Add DOC= field to CANARY token in source code
4. Run `canary doc update <PROJECT_KEY>-<ASPECT>-105` to register the hash
...
```
# After code changes
/canary.doc update <PROJECT_KEY>-<ASPECT>-105
```

**Agent should:**
1. Review what changed in the code
2. Update corresponding documentation files
3. Run `canary doc update <PROJECT_KEY>-<ASPECT>-105` to recalculate hashes
...
/canary.doc create <PROJECT_KEY>-<ASPECT>-XXX --type feature
```

**After /canary.plan:**
```
/canary.plan <PROJECT_KEY>-<ASPECT>-XXX
/canary.doc create <PROJECT_KEY>-<ASPECT>-XXX --type technical
```
...
**Arguments:**
- `<REQ-ID>`: Requirement identifier (e.g., <PROJECT_KEY>-<ASPECT>-105)
- `--type`: Documentation type (user, api, technical, feature, architecture)
- `--output`: Output file path

**Example:**
```bash
canary doc create <PROJECT_KEY>-<ASPECT>-105 --type user --output docs/user/auth.md
```
...
**Examples:**
```bash
canary doc status <PROJECT_KEY>-<ASPECT>-105
canary doc status --all
canary doc status --all --stale-only
```
...
3. **Example workflow:**
   ```bash
   # After implementing <PROJECT_KEY>-<ASPECT>-105
   canary doc create <PROJECT_KEY>-<ASPECT>-105 --type user --output docs/user/auth.md
...
1. **Create feature specification:**
   ```bash
   /canary.specify "Add user authentication"
   # Creates <PROJECT_KEY>-<ASPECT>-XXX
   ```

2. **Create feature documentation:**
   ```bash
   canary doc create <PROJECT_KEY>-<ASPECT>-XXX --type feature --output docs/features/auth.md
   # Fill in user stories and acceptance criteria
   ```

3. **Create technical design:**
   ```bash
   canary doc create <PROJECT_KEY>-<ASPECT>-XXX --type technical --output docs/technical/auth-impl.md
...
1. **Create all documentation:**
   ```bash
   canary doc create <PROJECT_KEY>-<ASPECT>-200 --type user --output docs/user/api-usage.md
   canary doc create <PROJECT_KEY>-<ASPECT>-200 --type api --output docs/api/endpoints.md
   canary doc create <PROJECT_KEY>-<ASPECT>-200 --type technical --output docs/technical/api-design.md
   ```

2. **Update CANARY token with multiple references:**
   ```go
   // CANARY: REQ=<PROJECT_KEY>-<ASPECT>-200; FEATURE="RestAPI"; ASPECT=API; STATUS=IMPL;
...
3. **Update all hashes together:**
   ```bash
   canary doc update <PROJECT_KEY>-<ASPECT>-200
   ```
